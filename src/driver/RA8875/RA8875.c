// original: https://github.com/lucafaccin/RA8875.git
...
static SPI_TypeDef *RA8875_SPI = NULL;
static GPIO_TypeDef *_reset_port = NULL;
static uint16_t _reset_pin = 0;
static GPIO_TypeDef *_wait_port = NULL;
static uint16_t _wait_pin = 0;
static RA8875_bpp_e _color_depth;
 ...
 */
/**************************************************************************/
uint8_t RA8875_begin (SPI_TypeDef *spi, GPIO_TypeDef *reset_port, uint16_t reset_pin, GPIO_TypeDef *wait_port, uint16_t wait_pin, RA8875_sizes_e s, RA8875_bpp_e bpp)
{
  RA8875_SPI = spi;
  ...
  if (RA8875_SPI == NULL || _reset_port == NULL) {return 0;}
 ...
 */
/**************************************************************************/
void RA8875_write_data(uint8_t data)
{
    uint8_t buf[2] = {RA8875_SPI_DATAWRITE, data};

    core_SPI_start(RA8875_SPI);
    core_SPI_read_write(RA8875_SPI, buf, 2, NULL, 0);
    core_SPI_stop(RA8875_SPI);
}
...
{
    uint8_t tx[2] = {RA8875_SPI_DATAREAD, 0x00};
    uint8_t rx[2] = {0};

    core_SPI_start(RA8875_SPI);
    core_SPI_read_write(RA8875_SPI, tx, 2, rx, 2);
    core_SPI_stop(RA8875_SPI);
 ...
 */
/**************************************************************************/
void RA8875_write_command(uint8_t reg)
{
    uint8_t buf[2] = {RA8875_SPI_CMDWRITE, reg};
...
{
    uint8_t tx[2] = {RA8875_SPI_STATUSREAD, 0x00};
    uint8_t rx[2] = {0};
